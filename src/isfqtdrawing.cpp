...
void Drawing::finalizeChanges()
{
  ...
  // stretch the bounding rectangle to start at 0,0 if we are not cropping
  // the output.
  if ( ! cropping_ )
  {
...
/**
 * Return the current cropping mode for this Drawing instance. By default cropping is enabled.
 *
 * If this value is True, then the QImage generated by getPixmap will be
 * cropped to the smallest rectangle that will contain the Ink image and the return value of 
 * getSize() will be the size of this rectangle.
 *
 * If this value is False, the original coordinates used when the Ink
 * was drawn are used. If you're creating an editor that can modify
 * existing Ink data, you should disable cropping otherwise there will be a discrepancy between
 * your coordinates you supply to addStroke and the coordinates used internally (offsets occur
 * during cropping).
 *
 * @return True if Cropping is enabled, False otherwise.
 */
bool Drawing::getCropping()
{
  return cropping_;
}
...
QPixmap Drawing::getPixmap( const QColor backgroundColor )
{
  ...
  // do this before we return a pixmap.
  finalizeChanges();
...
/**
 * Set cropping on or off.
 *
 * See getCropping() for a full explanation.
 *
 * @param on If True, cropping is enabled.
 */
void Drawing::setCropping( bool on )
{
  cropping_ = on;
  finalizeChanges();
}
